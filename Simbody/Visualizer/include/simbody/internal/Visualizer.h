...
for display and interaction through the simbody-visualizer. **/
...
The Simbody simbody-visualizer provides some user interaction of its own, for
...
/** Ask the simbody-visualizer to shut itself down immediately. This will cause the
display window to close and the associated process to die. This method returns
immediately but it may be some time later when the simbody-visualizer acts on the
instruction; there is no way to wait for it to die. Normally the simbody-visualizer
...
message to the simbody-visualizer when this %Visualizer object is destructed. 
...
/** These are the types of backgrounds the simbody-visualizer currently supports.
...
/** The simbody-visualizer may predefine some menus; if you need to refer to one
...
/** @name               simbody-visualizer display options
These methods provide programmatic control over some of the simbody-visualizer's
...
/** Change the title on the main simbody-visualizer window.\ The default title
...
/** Add a new pull-down menu to the simbody-visualizer's display. A label
...
When a user picks an item on a menu displayed in the simbody-visualizer, that 
...
/** Add a new slider to the simbody-visualizer's display.
...
@param value    the initial value of the slider, which must be between 
                min and max 
When a user moves a slider displayed in the simbody-visualizer, the new value 
